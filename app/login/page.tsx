...
export default function LoginPage() {
  ...
  const handleGuestContinue = () => {
    // Allows user to test offline/locally without an account
    router.replace('/onboarding');
  };
  ...
  return (
    <div className="min-h-[85vh] flex flex-col justify-between py-6 px-2 animate-in fade-in duration-200">
      ...
      <div className="space-y-3 text-center pt-4">
        ...
        <p className="text-xs text-subtleText max-w-xs mx-auto leading-relaxed pt-1">
          {mode === 'signin'
            ? 'Masuk untuk sinkronisasi latihan, beban, dan progres tubuh antar-perangkat.'
            : 'Buat akun baru untuk mulai melacak latihan dan mencapai tubuh atletis lean.'}
        </p>
      </div>
      ...
      <div className="bg-surface border border-surfaceBorder rounded-3xl p-5 shadow-2xl space-y-4 my-auto">
        ...
        <button
          ...
          className="w-full py-3 rounded-2xl bg-card hover:bg-surfaceBorder border border-surfaceBorder text-white text-xs font-bold flex items-center justify-center gap-2.5 active:scale-95 transition-all shadow-sm disabled:opacity-50"
        >
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-[10px] uppercase font-bold text-subtleText">atau email</span>
          <div className="flex-1 h-[1px] bg-surfaceBorder"></div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <button
            type="submit"
            disabled={isSubmitting}
            className="w-full py-3.5 rounded-2xl bg-primary hover:bg-primary-hover text-black font-extrabold text-xs flex items-center justify-center gap-2 shadow-lg shadow-primary/25 active:scale-95 transition-all disabled:opacity-50"
          >
            <span>{isSubmitting ? 'Memproses...' : mode === 'signin' ? 'Masuk ke Akun' : 'Daftar Sekarang'}</span>
            <ArrowRight className="w-4 h-4 stroke-[3px]" />
          </button>
        </form>
      </div>

      {/* 3. Bottom Guest / Offline Link */}
      <div className="text-center pt-2">
        <button
          type="button"
          onClick={handleGuestContinue}
          className="text-xs text-mutedText hover:text-white underline underline-offset-4 transition-colors"
        >
          Masuk sebagai Tamu (Mode Offline di Perangkat Ini)
        </button>
      </div>
    </div>
  );
